...
class DataTrainingArguments:
    ...
    image_only_pretrain: bool = field(default=False, metadata={"help": "If True, sets the data processing mode to pixels-only (mode 2)."})    
...
def main():
    ...
    def prepare_single_dataset(name, dataset_idx):
        ...
        # NEW LINE
        # The mode is now determined by the dataset index, but we can override it.
        # Mode 0: text+image, Mode 1: text-only, Mode 2: image-only
        # We'll set the default here and control it via shell script later if needed.
        mode_for_this_dataset = 0 
        if data_args.image_only_pretrain:
            mode_for_this_dataset = 2
        ds = ds.add_column("mode", [mode_for_this_dataset] * len(ds))
...
###
    if data_args.image_only_pretrain:
        rank_print("Freezing language-related parameters (embed_tokens and lm_token_head)...")
...
###
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        """
        Custom loss computation that lets the model handle its own logic.
        This works for both multimodal and pixels-only modes.
        """
