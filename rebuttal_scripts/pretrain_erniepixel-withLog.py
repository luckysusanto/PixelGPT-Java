...
HYPERPARAMETERS = {
    ...
    "patch_size": 16,
    "num_channels": 3,
    "per_device_train_batch_size": 4, # ori = 4
    "per_device_eval_batch_size": 4,
    "gradient_accumulation_steps": 4, # ori = 4
...
# =============================================================================
# LOGGING UTILITIES (added for OOM diagnostics)
# =============================================================================
class BatchTokenLogger:
    ...
    def __init__(self, csv_path: str, grad_accum_steps: int, rank: int, world_size: int):
        # Make csv_path rank-specific so every rank writes its own file without contention.
        # Preserves the caller's directory/basename; inserts "_rank{N}" before extension.
        ...
        # buffering=1 => line-buffered; safe against abrupt termination
    ...
    def log(self, input_ids_lengths: List[int], pixel_widths: List[int], batch_size: int):
        ...
        # Wall time (ISO with microseconds) lets us align rank 0 vs rank 1 CSVs post-hoc.
        wall = datetime.datetime.now().isoformat(timespec="microseconds")
        # GPU memory proximity signal. Safe even if CUDA not yet used.
        try:
        ...
        self.fh.flush()
        try:
            os.fsync(self.fh.fileno())  # force disk write; survives OOM kill
        except Exception:
            pass
        self.micro_step += 1
...
def compute_token_length_stats(dataset, text_column: str, label: str, source_filter=None):
    """Compute min/max/avg token length over a (possibly filtered) HF dataset.
    
    Accesses the raw column directly to avoid triggering set_transform.
    `source_filter` optionally restricts to samples where the 'mode' column matches —
    not used here since we compute stats before concatenation split becomes opaque,
    but kept general.
    """
    # Pull the column as a plain list (fast path on Arrow-backed datasets)
    ...
    mx = max(lengths)
    avg = sum(lengths) / n
    # Percentiles give a better picture of the tail than min/max alone
...
class SmartMultimodalCollator:
    ...
    image_size: List[int]
    num_channels: int
    # --- ADDED: optional logger; None => no logging overhead ---
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        # --- ADDED: collect per-sample stats for logging ---
        ...
        if has_pixel:
            ...
            for i, item in enumerate(batch):
                if item.get('pixel_values') is not None:
                    ...
                    _per_sample_pixel_widths.append(int(pixels.shape[2]))  # ADDED
        ...
        if has_text:
            ...
            for item in batch:
                if item.get('input_ids') is not None:
                    ...
                    _per_sample_token_lens.append(int(txt.shape[0]))  # ADDED
                else:
        ...
        # --- ADDED: flush one row per micro-batch ---
...
def main():
    ...
    # --- ADDED: compute token-length stats per dataset BEFORE concatenation/set_transform ---
    # Raw columns are still accessible as lists here.
    if rank == 0:
        rank_print("=" * 60)
    ...
    # --- ADDED: combined stats on the merged dataset ---
    if rank == 0:
    ...
    # --- ADDED: per-batch CSV logger (per-rank file: batch_token_stats_rank{N}.csv) ---
    ...
    # Each rank announces its own file so you can see in the log that both are writing.
    ...
    data_collator = SmartMultimodalCollator(
        ...
        image_size=model_args.image_size,
        num_channels=model_args.num_channels,
        batch_logger=batch_logger,  # ADDED
    )
    ...
    try:
        ...
        # --- ADDED: ensure CSV is flushed/closed even on OOM ---
        batch_logger.close()
