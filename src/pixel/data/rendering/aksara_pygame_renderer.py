...
class AksaraPyGameTextRenderer(TextRenderingMixin):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        JavaneseFontWrapper.set_default_resolution(dpi)
    ...
    def _render_single_word(
        ...
        # rect.top = self.pad_top + delta_top/self.pad_top
    ...
    def load_font(self) -> None:
        ...
        font = JavaneseFontWrapper(self.font_file, self.font_size)
...
class JavaneseFontWrapper:
    """Wrapper that mimics pygame.freetype.Font but uses Pillow for proper text shaping"""
    
    # Class variable to store DPI (default 72)
    _dpi = 72
    
    @classmethod
    def set_default_resolution(cls, dpi):
        """Set the default DPI for all font instances"""
        cls._dpi = dpi
    ...
    def __init__(self, font_path, size):
        ...
        # Calculate actual font size based on DPI
        actual_size = int(size * self._dpi / 72)
        
        self._pil_font = ImageFont.truetype(font_path, actual_size)
        self.style = pygame.freetype.STYLE_NORMAL
        self.fgcolor = (0, 0, 0)
    
    def render(self, text, fgcolor=None):
        """
        Render text with proper shaping using Pillow.
        Returns (surface, rect) — same format as pygame.freetype.Font.render()
        """
        ...
        # Normalize color
        if isinstance(fgcolor, pygame.Color):
            color = (fgcolor.r, fgcolor.g, fgcolor.b, fgcolor.a)
        else:
            color = fgcolor

        try:
            # Pillow gives bbox = (left, top, right, bottom) relative to baseline
            ...
            # Draw text so that baseline aligns properly within the image
            # bbox[1] is how far text extends above baseline (can be negative)
            draw_x = padding - bbox[0]
            draw_y = padding - bbox[1]  # ensures proper baseline placement
            draw.text((draw_x, draw_y), text, font=self._pil_font, fill=color)

            # Convert to Pygame surface
            text_surface = pygame.image.fromstring(img.tobytes(), img.size, img.mode)

            # Create rect where top reflects how far text extends above baseline
            rect = pygame.Rect(0, bbox[1] - padding, text_width + padding * 2, text_height + padding * 2)
    ...
    def get_rect(self, text):
        """
        Get the rect for the given text.
        Returns a pygame.Rect similar to freetype.Font.get_rect(),
        where rect.top is relative to the baseline (can be negative).
        """
        if not text or text.isspace():
            return pygame.Rect(0, 0, self.size // 2, self.size)
        ...
        try:
            ...
            print(f"Error getting rect for text '{text}': {e}")
