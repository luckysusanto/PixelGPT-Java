...
# coding=utf-8
"""
DualGPT Pretraining Script.
Features:
1. Constant Learning Rate with Warmup.
2. AUTOMATIC EPOCH SPLITTING: If you ask for 3 epochs, it creates 3 sequential folders
   (epoch-0, epoch-1, epoch-2) automatically in one run.
"""
...
class CustomTrainingArguments(TrainingArguments):
    ...
    # NOTE: Set this to the TOTAL number of epochs you want (e.g. 3).
    # The script will split this into 3 separate folders.
...
def get_next_epoch_paths(base_output_dir):
    """
    Scans base_output_dir for 'epoch-N' folders.
    Returns:
        latest_model_path (str or None): Path to the highest epoch folder found (source).
        new_output_dir (str): Path to the next epoch folder (destination).
    """
...
# MAIN TRAINING LOOP
# -------------------------
def run_single_epoch(model_args, data_args, training_args, datasets_to_mix, tokenizer, resume_checkpoint=None):
    """
    ...
    # If resuming crash, use that. If continued epoch, use model_name_or_path.
    ...
    # 2. Data
    logger.info("Mixing datasets for this epoch...")
...
def main():
    ...
    # --- Prepare Datasets (Load once) ---
    # We load them here so we don't re-download every loop, 
    # but we shuffle inside the loop.
    datasets_to_mix = []
    ...
    # 1. Determine how many epochs user wants total
    total_requested_epochs = int(training_args.num_train_epochs)
    
    # 2. Force the internal Trainer to only run 1 epoch at a time
    ...
    for i in range(total_requested_epochs):
        ...
        # A. Resolve Folders (epoch-N -> epoch-N+1)
        latest_model_path, new_output_dir = get_next_epoch_paths(base_output_dir)
        
        # B. Check for Crash Recovery
        # If the destination (new_output_dir) already has checkpoints, we might be resuming a crashed run
        ...
        # If we found a previous epoch, load from it. 
        # (Unless we are resuming a crash in the target folder, in which case Trainer handles logic via resume_from_checkpoint)
        if latest_model_path and not resume_checkpoint:
            ...
            # First run ever, and no resume. Use original args.
            pass
