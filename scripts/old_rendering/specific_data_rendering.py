...
def process_parquet_to_dataset(filepath, custom_text_renderer, tokenizer, tokenizer_name, batch_path, batch_size=5000, debug=False):
    ...
    for i, row in enumerate(tqdm(df.itertuples(), total=len(df))):
        ...
        tokenizer_encoding = tokenizer.encode(row.tokenized_text, is_split_into_words=True)
        records.append({
            'text_id': row.doc_id,
            'chunk_id': row.chunk_id,
            'pixel_values': pixel_encoding.pixel_values,
            f'{tokenizer_name}_token_ids': tokenizer_encoding,
            'text': row.chunk_text
        })
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tokenizer_name', type=str, default='Custom')
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_path, force_download=True, add_eos_token=True)
...
- `<tokenizer_name>_token_ids`: Token IDs from grapheme-based tokenizer (with EOS token)
...
"""
    ...
    # Push to hub
    login('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')
    print('Pushing to hub...')
    
    repo_name = f"Exqrch/{args.lang}-{tokenizer_name}-pixelgpt" + ("-debug" if args.debug else "")
